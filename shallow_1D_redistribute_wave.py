...
def shallow_fwave_dry_1d(q_l, q_r, aux_l, aux_r, problem_data):
    ...
    for i in range(num_rp):        
        ...
        if i == loc_bar:
            ...
            amdq[:,i] += amdq_small_l
            apdq[:,i] += apdq_small_l
            amdq[:,i+1] += amdq_small_r
            apdq[:,i+1] += apdq_small_r
            amdq[:,i-1] += amdq_LL
            apdq[:,i+2] += apdq_RR
...
def barrier_solver(qLL, qL, qR, qRR, alpha, barrier_height):
   ...
   # Riemann subproblem 1:
    wave_wall, s_wall, amdq_wall, apdq_wall  = redistribute_fwave(qL+(1-alpha)*qLL, qR+(alpha)*qRR,0,0,barrier_height,0.001,9.8,50)
    
   # Riemann subproblem 2:
    fwave2, s2, amdq2, apdq2 = shallow_fwave_dry_1d(qLL, (1-alpha)*qR + (alpha)*qL,0,0,problem_data)
    
   # Riemann subproblem 3:
    fwave3, s3, amdq3, apdq3 = shallow_fwave_dry_1d((1-alpha)*qR + (alpha)*qL, qRR, 0,0,problem_data)
    ...
    LST = qL+(1-alpha)*qLL
    RST = qR+alpha*qRR
    hstar,_,_,_,_ = riemanntype(LST[0], LST[0], LST[1]/LST[0], -LST[1]/LST[0],50,.001,9.8)
    limiting_constant = max(0, 1-(barrier_height/hstar))
     # from right to left
    hstar2,_,_,_,_ = riemanntype(RST[0],RST[0],-RST[1]/RST[0], RST[1]/RST[0], 50,0.001,9.8)
   ...
   # fwave for barrier cell:
    ...
    apqd_small_r = 0.9*apdq_wall + (limiting_constant)*apdq2
